...
public class RegistryHandler {
	...
	public static void onItemRegister(RegistryEvent.Register<Item> event) {
		...
		event.getRegistry().registerAll(Minerals.ITEMS.toArray(new Item[0]));
		...
		event.getRegistry().registerAll(Food.ITEMS.toArray(new Item[0]));
		event.getRegistry().registerAll(Berries.ITEMS.toArray(new Item[0]));
	}
	...
	public static void onModelRegister(ModelRegistryEvent event) {
		...
		for(Item item : Food.ITEMS) {
			if(item instanceof IHasModel) {
				((IHasModel)item).registerModels();
			}
		}
		for(Item item : Berries.ITEMS) {
			if(item instanceof IHasModel) {
				((IHasModel)item).registerModels();
			}
		}
